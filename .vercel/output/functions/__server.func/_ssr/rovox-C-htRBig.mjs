import { i as FINDINGS, n as BRIEF_TAGS } from "./research-BbnSBAkh.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/rovox-C-htRBig.js
var ROVOX_STORAGE = {
	card: "rs-rovox-card",
	docs: "rs-rovox-docs"
};
...
function journalDigest() {
	return `FINDINGS\n${FINDINGS.map((f) => `${f.kicker}: ${f.text}`).join("\n")}\n\nBRIEF\n${BRIEF_TAGS.en.map((t) => `${t.title}: ${t.body}`).join("\n\n")}`;
}
function searchJournal(query, limit = 4) {
	const chunks = journalDigest().split(/\n\n+/).map((c) => c.trim()).filter(Boolean);
	...
	}).filter((row) => row.score > 0).sort((a, b) => b.score - a.score).slice(0, limit).map((row) => row.chunk.slice(0, 900));
}
var FALLBACK_INTRO = {
...
//#endregion
export { searchJournal as a, ROVOX_STORAGE as i, DEFAULT_DOCS as n, FALLBACK_INTRO as r, DEFAULT_CARD as t };
